-- V223: restore the troubleshooting evidence tool after V208 reused the
-- historical V173 id, and make the dedicated troubleshooting Agent available
-- in the default workspace. The Agent is deliberately limited to the one
-- read-only evidence tool; the server-side triage scope remains the final
-- security boundary.
...
ON DUPLICATE KEY UPDATE
    ...
    description=VALUES(description), tool_type=VALUES(tool_type),
    bean_name=VALUES(bean_name), icon=VALUES(icon), enabled=VALUES(enabled),
    builtin=VALUES(builtin), update_time=VALUES(update_time), deleted=VALUES(deleted);
...
WHERE agent_id = @troubleshooting_agent_id
...
VALUES
    (1000000951, @troubleshooting_agent_id, 'TroubleshootingEvidenceTool',
     TRUE, NOW(), NOW(), 0)
ON DUPLICATE KEY UPDATE
    enabled=VALUES(enabled), update_time=VALUES(update_time), deleted=VALUES(deleted);
