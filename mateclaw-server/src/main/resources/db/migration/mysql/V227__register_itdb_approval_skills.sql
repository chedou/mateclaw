...
ON DUPLICATE KEY UPDATE
    name=VALUES(name), description=VALUES(description), skill_type=VALUES(skill_type),
    icon=VALUES(icon), version=VALUES(version), author=VALUES(author),
    config_json=VALUES(config_json), enabled=VALUES(enabled), builtin=VALUES(builtin),
    tags=VALUES(tags), workspace_id=VALUES(workspace_id), update_time=VALUES(update_time),
...
FROM mate_agent a JOIN mate_skill s ON s.id = 2091870934831804418
WHERE a.workspace_id = 1 AND a.name = 'SXF-ITDB SQL审批安全官' AND s.deleted = 0
LIMIT 1
ON DUPLICATE KEY UPDATE enabled=VALUES(enabled), update_time=VALUES(update_time), deleted=VALUES(deleted);

INSERT INTO mate_agent_skill
    (id, agent_id, skill_id, enabled, config_json, create_time, update_time, deleted)
SELECT 1000000962, a.id, s.id, TRUE, NULL, NOW(), NOW(), 0
FROM mate_agent a JOIN mate_skill s ON s.id = 2091870991194861569
WHERE a.workspace_id = 1 AND a.name = 'SXF-ITDB SQL审批安全官' AND s.deleted = 0
LIMIT 1
ON DUPLICATE KEY UPDATE enabled=VALUES(enabled), update_time=VALUES(update_time), deleted=VALUES(deleted);
...
FROM mate_agent a JOIN mate_skill s ON s.id = 2091871049277583362
WHERE a.workspace_id = 1 AND a.name = 'SXF-ITDB SQL审批安全官' AND s.deleted = 0
LIMIT 1
ON DUPLICATE KEY UPDATE enabled=VALUES(enabled), update_time=VALUES(update_time), deleted=VALUES(deleted);
...
FROM mate_agent a JOIN mate_skill s ON s.id = 2091871106345283585
WHERE a.workspace_id = 1 AND a.name = 'SXF-ITDB SQL审批安全官' AND s.deleted = 0
LIMIT 1
ON DUPLICATE KEY UPDATE enabled=VALUES(enabled), update_time=VALUES(update_time), deleted=VALUES(deleted);
