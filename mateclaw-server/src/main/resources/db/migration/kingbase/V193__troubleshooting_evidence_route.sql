...
-- Exactly one live route per (workspace, system, signal kind): two rows answering
-- the same question would let insertion order decide which source is queried.
CREATE UNIQUE INDEX IF NOT EXISTS uk_ts_evidence_route
    ON mate_troubleshooting_evidence_route(workspace_id, system, signal_kind);
CREATE INDEX IF NOT EXISTS idx_ts_evidence_route_system
    ON mate_troubleshooting_evidence_route(workspace_id, system);
