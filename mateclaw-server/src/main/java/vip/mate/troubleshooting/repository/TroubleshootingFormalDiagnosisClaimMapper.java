...
@Mapper
public interface TroubleshootingFormalDiagnosisClaimMapper
        extends BaseMapper<TroubleshootingFormalDiagnosisClaimEntity> {
    ...
    @Select("""
            SELECT id, workspace_id, dedup_key, claim_token, status, diagnosis_id,
              ...
              FROM mate_troubleshooting_formal_diagnosis_claim
             WHERE workspace_id = #{workspaceId}
               AND dedup_key = #{dedupKey}
             LIMIT 1
            """)
    TroubleshootingFormalDiagnosisClaimEntity findByKey(
    ...
    @Delete("""
            DELETE FROM mate_troubleshooting_formal_diagnosis_claim
             WHERE workspace_id = #{workspaceId}
               AND dedup_key = #{dedupKey}
               AND status = 'PROCESSING'
               AND claim_token = #{claimToken}
