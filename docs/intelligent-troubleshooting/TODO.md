...
- [x] V222 将安全规范化的完整排障轮次复用 MySQL 聊天表持久化；先写不含原文的
  `PENDING` 消息对，再在会话锁内原位完成，并以指针台账保证响应丢失/并发重试不重复。
  普通异常转成 `FAILED_RETRYABLE`；刷新后从服务端 metadata 恢复原 `clientTurnId` 再重试。
- [x] 普通聊天、结构化排障、Diagnosis 绑定追问均以 MySQL 为真源；追问保存
  安全规范化问答和诊断关联元数据，补充材料只保存不可变待验证回执，不把
  密钥、个人信息或整段原始日志复制进 `mate_message`。
- [ ] 运行态仍需完成 T7 `20 / 20`、保存真实试点范围、执行当前 owner 验收并部署 V222。
