...
| # | 事项 | 卡在什么上 | 位置 |
|---|---|---|---|
| 1 | **T7 内网核实**：真实 Guance measurement / 字段 / 阈值 | 需要内网窗口 + owner 本人，**不是代码**。验收接缝、工作台向导、窗口预检均已就绪 | §5 T7 |
...
| 5 | T8 历史样本 20–30 条 + 性能基线 | 依赖 1。系统置信度与质量统计口径已补齐，缺的是 owner 验收后的真源样本 | §5 T8 |
...
1. 在窗口外准备 20–30 条 `t7-recording-window-plan.v1` 本地目标清单，再在目标环境运行
...
- [x] 窗口预检 `scripts/troubleshooting-t7-preflight.sh`（2026-08-02）。
      ...
      20–30 条种子目标清单 → 验收状态 → 真源采样闸门；卡住时把**服务端自己的 blockers 原样打出来**，
      再给下一步动作。四条刻意的约束：
      ...
      - **没有 20–30 条清单不报窗口就绪。** `t7-recording-window-plan.v1` 只接受冻结 D1 成员、
        精确故障时间和安全 lookup 元数据；不导入 Playbook、不调用 Guance、不接受聚合结果、DQL、
        原始日志或凭据。少于 20 条的单次 Demo、超过 30 条、重复/已有/清单外 selector 均在窗口外阻断。
