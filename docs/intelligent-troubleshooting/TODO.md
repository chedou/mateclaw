...
> 更新时间：2026-08-13（Owner 标准查询登记操作检查点）
>
...
- 统一入口的“配置试点”现在会一键进入并展开设置，不再要求管理员进入评估页后再点一次。页面已实测当前
  Workspace 只有 **`1 / 3`** 名成员；成员与角色不满足时，“去补齐成员与角色”是唯一前置动作。本轮没有
  新增人员或保存计划。
...
- [ ] 非阻塞重构债：进度投影和最终规范化已经共享基础字段校验，但仍分别声明一次“15 个字段如何映射”；
  下次修改 Owner 合同时应由同一份类型化字段描述派生两者，避免新增字段后页面进度与最终校验漂移。
