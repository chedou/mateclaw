...
> 状态：**现行设计，架构师评审通过（P1 实施门）· 当前修订 v4.3 / 蓝图 v0.14**
>
...
| `conversationRef` / `reporterRef` | `ChannelMessage.chatId / senderId`、`ChannelSessionStore` | Router 在 pre-route 接管前保存带 channelId 的原通道路由；Intake 直接取标准消息字段，不新建会话表 |
...
**入站可以先行**：普通消息入站只用 Router pre-route handler，不受 card renderer 形状影响，所以 P3 可以先打通
