...
> 普通消息 pre-route、IntakeSession、READY 异步调查、纯文本 BusinessSummary 与 Web 深链已落地。
>
...
**v0.15 实现状态**：READY 与数据库调查任务同事务提交；租约 worker 复用既有只读调查服务，
...
| P3（前半段已完成） | 通道交付 renderer 消费 `BusinessSummary` 生成纯文本并原路回复；投影本身不变。关闭且 outcome 已登记后的通知、交互卡片仍待实现 |
