...
T59 同一告警窗口的失败请求分组（2026-08-17）：

- 用户提供的 `CSDP / csdp-task / CTI 创建会话失败` 告警在人工只读核对中暴露了原三段
  Evidence Spine 的信息损失：3 条外层 `701018` 日志实际来自 2 个独立请求，且分别命中
  「必填业务编码缺失」与「下游记录 / 缓存未找到」线索。仅取最新关联 ID 会隐藏前一种失败。
- CTI 录制种子新增 **v2 候选**：保留 `log_search → log_trace_bundle → contrast_sample`，再追加可选的
  `cti_failure_pattern_scan`。第四步由服务端冻结三个有界只读查询，在 Adapter 内存中将「外层 701018 失败请求集」分别与两个特征候选集求交，随后丢弃所有关联 ID；
  `EvidenceResult` 只携带失败请求总数、特征并集的已识别请求数与两个已审核特征计数，不返回或持久化关联 ID、原始日志、DQL、端点或凭据。同一请求可保留多条并列线索，未归类数由特征并集而不是分类计数之和计算；计数不可复算时整体 fail closed。
- 详情投影新增「同一批告警里有几类问题」：先说独立失败请求数，再列每类线索与未归类数。
  文案明确这是相关线索，需人工确认最终责任点，不将多种失败压成唯一根因。
- 既有三步 Playbook 和历史详情保持兼容；既有 CTI v1 与历史 Diagnosis 不回写。v2 必须完成回放、
  人工审核和 active-approved 切换后才会进入在线命中路；本次人工真源核对不冒充新版本已投产，
  T7 Owner 正式录制仍为 **`0 / 20`**。
- 对 2026-08-17 15:29–15:44 的历史窗口完成了真实 Guance 只读合同验收：三组查询均返回
  `time + @trace_id` 的有界结果，Adapter 等价集合交集得到失败请求 `2`、已识别请求 `2`、
  「必填业务编码缺失」`1`、「下游记录 / 缓存未找到」`1`。验收过程只输出聚合数量，未打印或
  持久化关联 ID、日志正文、DQL、端点和凭据；这仍是候选合同验收，不等于 v2 已审核上线。
...
T61 排障详情第一性原理瘦身（2026-08-18）：

- 三线首屏改成一张「故障判定单」，内容顺序固定为「当前能确认的结论 → 系统为什么这样判断 →
  人现在只需要做的一件事」。原先并排的关键证据、未知项、下一步、五问进度和重复场景说明不再同时
  占据首屏；候选线索、耗时与完整技术记录保留，但默认折叠。
- “为什么”只显示三段可审计信息：事实来自告警上报、真实只读观测还是录制回放；本次实际记录的
  关键依据；以及当前不能继续定因的边界。页面不生成内部思维链，也不拿缺失证据补写根因。
- 修正 `HYPOTHESIS + CONFIRMED` 的误导语义：状态显示为「候选方向已人工确认」，不再显示成笼统
  「已确认」；下一步改为继续核实未确认的上游根因。只有 `LOCATED + CONFIRMED` 才显示
  「根因已人工确认」。这只改变展示和人工指引，不改写已持久化 Diagnosis。
- 已完成真实浏览器双视角验收：iCare 演练单 `diag-53eb11b64ad14a47a64dab9d602830cb`
  在三线视角明确区分直接失败点与上游未知原因，二线视角继续只显示影响、升级和交接。前端全量
  `561 / 561`、`vue-tsc --noEmit`、生产构建和 `git diff --check` 通过；本单仍为演练，T7 仍是
  **`0 / 20`**。

T62 三线详情的单焦点设计（2026-08-18）：

- 按「先给答案，再给边界，最后给动作」重做三线首屏。主标题只回答根因是否找到；其下只保留
  「已经知道 / 还不知道」两行，不再让三线先阅读三列步骤、置信标签和系统术语。
- 「为什么系统只说到这里」改成一条由结构化 `EvidenceBasis` 生成的简短解释；事实来源、可信度、
  候选分组和阶段耗时全部进入「查看判断依据」。完整证据与技术记录仍是第二层折叠，不删除审计事实。
- `HYPOTHESIS` 首屏固定使用「尚未找到根因」，不会把 `rootCause` 字段中的直接失败点误当成最终根因；
  已知事实优先显示持久化 `keyEvidence`，未知项由结论类型和候选数量确定，不解析中文前缀改变语义。
- 详情路由自动把智能排障二级导航收成图标栏，返回列表后恢复用户原有导航偏好。详情工具栏与判定卡
  同步压缩，1280×720 首屏已经能够看到「接下来做什么」和主要操作按钮。
- MySQL 本地服务 health 为 `UP`。使用演练单 `diag-53eb11b64ad14a47a64dab9d602830cb`
  完成登录态浏览器验收，控制台 `0 error`；前端全量 `563 / 563`、`vue-tsc --noEmit`、生产构建和
  `git diff --check` 通过。该项只改变展示，不改变 Diagnosis、权限、取证或 T7，T7 仍为 **`0 / 20`**。
