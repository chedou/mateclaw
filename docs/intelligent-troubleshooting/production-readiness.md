...
**D20 不再被当成通用平台投产的前置条件。** 没有已审核场景规则的结构化告警，
可以走正式 `OPEN_DISCOVERY`：先校验精确试点范围与当前 Guance owner 验收，再由
服务端有界规划器使用只读工具取证，最后只输出候选原因或明确弃权；该路径不调用
开放式 Agent，也不执行生产写。D20 仅继续约束需要 `scenarioKey` 精确授权的场景级能力。

**当前实际挡住通用平台正式放量的是真实授权尚未闭环**：20–30 条真实
录制目标要由懂 Guance 的 owner 核实，并在内网窗口完成当前 binding 指纹验收。
CTI 创建会话失败和 ITGW 904003 拥有历史真源运行记录，证明查询合同可运行；它们不构成
当前正式通道的许可。ITGW 仍等待 T7 批次与 owner 验收；CTI 还必须先补齐 D20 场景级绑定与验收。

首版通用正式入口是「排障工作台 → 新建排障单」和同一 Incident API。
对话补问入口在共享的正式 claim / 持久化接缝完成前仍不放行，不用演练记录代替正式调查。
...
历史真源   CTI + ITGW     ← 证明查询可运行，不是当前正式许可
场景授权   CTI 待 D20    ← 完成前 UI 只能创建演练单
通用调查   代码闭环已就绪   ← 不依赖 D20，仍要求试点 + T7 + 当前 owner 验收
代码侧闸门 已就绪          ← 仍不会绕过 owner 和真实样本
```

运行环境还必须显式启用
`SPRING_PROFILES_ACTIVE=mysql,csdp-guance-evidence-pilot`。这不会放开场景级正式通道；
它只让通用观测云调查在通过上述门禁后有可执行的只读工具和硬预算。
...
- **通用平台**：是第一批上线主线。精确 `system / service` 已纳入试点且当前 Guance
  验收有效时，未知告警可正式运行有界只读调查；没有证据就弃权。
- **2 条业务场景**：是随后逐步增强的精确能力；ITGW 错误码规则可在 T7 通过后进入正式通道，CTI 场景规则在 D20 完成前只允许演练。不同时扩 K8s、HCI 和多 Agent。
- **20 份录制目标**：是 T7 批量验收的最小分母，用于证明平台不是只会演示两个案例。

| 步骤 | 谁来做 | 现在就做什么 | 完成标志 |
|---|---|---|---|
| 1. 冻结试点范围 | 业务负责人 + 二线 + 三线 | 第一批影子试用只放 CTI 和 ITGW；指定 1 名二线使用者、1 名三线复核人和 1 名 Guance owner；保存试点版本。ITGW 通过 T7 后再建正式单，CTI 先保持演练 | 人、场景和试运行时间已确定；页面不会把 CTI 演练单标为正式样本 |
...
- 第一批影子试用只放已有真源运行记录的 **CTI 创建会话失败**和 **ITGW 904003**。
- **ITGW** 等待 T7 全局批次和当前 binding 的 owner 验收；**CTI** 除此之外还等待 D20 场景级资产、指纹与验收。D20 完成前，CTI 正式请求 fail-closed，页面仅创建演练单。
