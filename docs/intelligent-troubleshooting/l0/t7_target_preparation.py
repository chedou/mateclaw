...
def render_markdown(report: Mapping[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "2. Owner 为 `NEEDS_OWNER_CONTRACT` 核对真实运行 service、服务端查询合同、安全 search term、判据/规则和历史故障时间。",
            "3. 将完整、安全、未验证的 candidate 与当前三份 bindingRef 冻结进 `guance-recording-targets.json`；本报告不生成该文件。",
            "4. 重启运行服务并执行 `T7_SEED_PLAN_FILE=<受控计划> ./scripts/troubleshooting-t7-preflight.sh`。只有运行时返回 20–30 个可执行目标才可约窗口。",
            "5. 窗口里由 owner 完成清单并提交 `ACCEPTED`，随后一次灌入 20–30 份 D19 聚合正例。",
            "",
