...
>   容器起来后 `/actuator/health` 返回 `UP`。当前源码共 216 个 Flyway 迁移
>   （最高 V222）；V218–V222 已通过三方言形状与 H2 执行验证，
>   测试环境切换仍需按本文维护窗流程执行。
...
3. 只读检查当前 Flyway 版本；无待执行迁移可直接继续，或只允许内容哈希完全一致的
   `V204 → V217` 基础包与 `V217 → V222` 正式排障包；
...
- `V217 → V222`：5 个正式排障迁移，增加评估样本来源字段、正式诊断 claim 表、通用正式调查的来源验收审计字段、诊断后补充材料的不可变安全台账，以及排障对话轮次的幂等指针台账。

两个包的文件范围和组合 SHA-256 均在 `Jenkinsfile.test-env` 冻结。可从 V204
一次升至 V222，也可从 V217 只执行 V218–V222；V218–V221 中断后只允许继续完成同一审核包。
