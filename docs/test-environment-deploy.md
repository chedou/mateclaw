...
>   容器起来后 `/actuator/health` 返回 `UP`。当前源码共 215 个 Flyway 迁移
>   （最高 V221）；V218–V221 已通过三方言形状与 H2 执行验证，
>   测试环境切换仍需按本文维护窗流程执行。
...
3. 只读检查当前 Flyway 版本；无待执行迁移可直接继续，或只允许内容哈希完全一致的
   `V204 → V217` 基础包与 `V217 → V221` 正式排障包；
...
- `V217 → V221`：4 个正式排障迁移，增加评估样本来源字段、正式诊断 claim 表、通用正式调查的来源验收审计字段，以及诊断后补充材料的不可变安全台账。

两个包的文件范围和组合 SHA-256 均在 `Jenkinsfile.test-env` 冻结。可从 V204
一次升至 V221，也可从 V217 只执行 V218–V221；V218–V220 中断后只允许继续完成同一审核包。
