...
>   容器起来后 `/actuator/health` 返回 `UP`。当前源码最高迁移为 V228；
>   V218–V228 已纳入测试环境迁移门禁，
>   测试环境切换仍需按本文维护窗流程执行。
...
先确认动态链接的宿主 `libseccomp >= 2.5.0`，再在基于 Moby `v18.06.0-ce`
官方默认规则、仅补充 `clone3` 的审核 profile 下实际创建 Node Worker 线程。低于 2.5.0 的
libseccomp 无法解析 `clone3`，即使 profile 写入该名称也不会生效，因此流水线会在拉镜像前明确停止。
...
2. 在旧 Docker 上先核验 `libseccomp >= 2.5.0`，再用正式运行基础镜像和已审核的 clone3 seccomp 执行线程探针，最后从该提交构建带不可变版本号的 Docker 镜像；
...
5. 新版本必须同时通过健康检查、精确 Git 版本校验和数据库 `V228` 校验，且失败迁移数必须为 0；
   否则恢复旧容器并保留 MySQL 备份。
...
本任务通常只允许数据库已无待执行迁移时发布应用。已审核四个连续例外：
...
- `V225 → V228`：启用正式只读排障机器人，注册并绑定 4 个稳定 ID 的 ITDB 审批 Skills，清理同名旧 Skill 对排障与审批入口的干扰。

四个包的文件范围和组合 SHA-256 均在 `Jenkinsfile.test-env` 冻结。可从 V204
一次升至 V228，也可从 V217、V218–V227 的已审核恢复点继续至 V228；中断后只允许继续完成同一审核包。
