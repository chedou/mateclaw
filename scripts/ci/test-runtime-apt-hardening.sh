...
keyring_download_line="$(line_of 'https://archive.ubuntu.com/ubuntu/project/ubuntu-archive-keyring.gpg')"
...
[[ -n "${keyring_download_line}" ]] \
  || fail "runtime stage must download Ubuntu's canonical archive keyring"
...
(( keyring_download_line < keyring_checksum_line )) \
  || fail "archive keyring download must happen before checksum verification"
...
grep -Fq -- '--connect-timeout 15 --retry 5 --retry-all-errors' <<<"${runtime_block}" \
  || fail "archive keyring download must use bounded retries"
