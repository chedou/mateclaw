  ...
  <details v-if="developer && business && current" class="developer-fold">
    ...
    <div class="developer-body">
      ...
      <div class="convergence-grid">
        <section class="trace-summary">
          ...
          <div v-if="developer.contrast.available" class="contrast-diff-section">
            ...
            <div ref="diffContainerRef" class="contrast-diff-container" style="height: 300px;" />
      ...
      <section class="evidence-timeline">
        ...
        <div v-if="!filteredSteps.length" class="empty-evidence">当前 Diagnosis 没有可展示的证据或判据步骤。</div>
      </section>
...
.developer-fold>summary { display:flex; align-items:center; gap:12px; padding:16px 20px; list-style:none; cursor:pointer; user-select:none; }
.developer-fold>summary::-webkit-details-marker { display:none; } .developer-fold>summary>div b,.developer-fold>summary>div small { display:block; } .developer-fold>summary>div b { font-size:var(--mc-text-sm); } .developer-fold>summary>div small { margin-top:3px; color:var(--mc-text-secondary); font-size:10px; }
.developer-fold>summary>span:last-child { margin-left:auto; color:var(--mc-text-secondary); font-size:10px; } .fold-caret { width:0; height:0; border-top:5px solid transparent; border-bottom:5px solid transparent; border-left:6px solid var(--mc-text-tertiary); transition:transform .18s; } .developer-fold[open] .fold-caret { transform:rotate(90deg); }
.developer-body { display:grid; grid-template-columns:minmax(0,1.65fr) minmax(300px,.75fr); gap:20px; padding:22px; border-top:1px solid var(--mc-border); background:var(--mc-bg-elevated); } .developer-body>.route-card,.developer-body>.convergence-grid { grid-column:1/-1; margin-top:0; }
...
.developer-section-head { display:flex; align-items:flex-start; justify-content:space-between; gap:16px; } .developer-section-head h3 { margin:5px 0 0; font-size:var(--mc-text-sm); }
.developer-section-head>span { padding:3px 8px; border-radius:var(--mc-radius-xs); color:var(--mc-status-info-text); background:var(--mc-status-info-bg); font-size:10px; }
.route-card { align-self:start; padding:15px; border:1px solid var(--mc-border); border-radius:var(--mc-radius-sm); background:var(--mc-bg-muted); }
.route-card span { display:block; color:var(--mc-text-tertiary); font-size:9px; font-weight:750; letter-spacing:.1em; text-transform:uppercase; }
.route-card b { display:block; margin:7px 0; font-size:12px; } .route-card code { color:var(--mc-primary); font-size:10px; word-break:break-all; }
.convergence-grid { display:grid; grid-template-columns:minmax(0,1.6fr) minmax(270px,.8fr); gap:14px; margin-top:14px; } .trace-summary,.draft-summary { padding:18px; border:1px solid var(--mc-border); border-radius:var(--mc-radius-sm); }
.section-head { display:flex; align-items:flex-start; justify-content:space-between; gap:16px; } .section-head h3 { margin:5px 0 0; font-size:var(--mc-text-sm); } .section-head>code { color:var(--mc-primary); font-size:10px; }
.hop-line { display:flex; align-items:stretch; gap:8px; margin-top:17px; } .hop { flex:1; padding:10px; border:1px solid var(--mc-border); border-radius:var(--mc-radius-xs); background:var(--mc-bg-elevated); }
.hop>span { display:inline-grid; place-items:center; width:18px; height:18px; border-radius:50%; color:var(--mc-text-inverse); background:var(--mc-primary); font-size:9px; } .hop b,.hop small { display:block; margin-top:5px; font-size:var(--mc-text-xs); } .hop small { color:var(--mc-text-secondary); }
.hop.anomalous { border-color:var(--mc-danger-border); background:var(--mc-status-error-bg); } .hop.anomalous>span { background:var(--mc-danger); }
...
.contrast-row { display:flex; align-items:center; gap:9px; flex-wrap:wrap; margin-top:14px; padding:10px 12px; border-radius:var(--mc-radius-sm); background:var(--mc-status-success-bg); font-size:10px; }
.contrast-row>span { color:var(--mc-text-secondary); } .contrast-row em { color:var(--mc-text-tertiary); font-style:normal; } .contrast-row .baseline { color:var(--mc-success); } .contrast-row small { flex-basis:100%; color:var(--mc-text-secondary); } .contrast-row.unavailable { color:var(--mc-warning); background:var(--mc-status-warning-bg); }
.draft-state { padding:2px 7px; border-radius:var(--mc-radius-xs); color:var(--mc-status-purple-text); background:var(--mc-status-purple-bg); font-size:9px; font-weight:700; } .draft-summary ol { margin:14px 0 9px; padding-left:20px; color:var(--mc-text-secondary); font-size:var(--mc-text-xs); line-height:1.6; } .draft-summary>small { color:var(--mc-text-secondary); font-size:10px; }
.evidence-timeline { min-width:0; } .evidence-step { display:grid; grid-template-columns:74px 20px minmax(0,1fr) auto; gap:8px; padding-top:17px; } .evidence-step time { padding-top:2px; color:var(--mc-text-tertiary); font-family:var(--mc-mono,monospace); font-size:9px; }
.step-line { position:relative; display:flex; justify-content:center; } .step-line::after { content:''; position:absolute; top:10px; bottom:-18px; width:1px; background:var(--mc-border-light); } .evidence-step:last-child .step-line::after { display:none; }
...
.evidence-step.excluded .step-line i { background:var(--mc-success); box-shadow:0 0 0 4px var(--mc-success-light); }
...
@keyframes pulse-red { 0%,100% { box-shadow:0 0 0 4px var(--mc-danger-light); } 50% { box-shadow:0 0 0 8px rgba(224,90,74,.1); } }
.evidence-step b { font-size:12px; } .evidence-step p { margin:4px 0 6px; color:var(--mc-text-secondary); font-size:var(--mc-text-xs); line-height:1.55; } .evidence-step code { color:var(--mc-primary); font-size:9px; }
.tone-label { align-self:start; padding:2px 6px; border-radius:var(--mc-radius-xs); color:var(--mc-text-secondary); background:var(--mc-bg-muted); font-size:9px; } .evidence-step.anomaly .tone-label { color:var(--mc-danger); background:var(--mc-status-error-bg); } .evidence-step.excluded .tone-label { color:var(--mc-success); background:var(--mc-status-success-bg); } .evidence-step.unevaluated .tone-label { color:var(--mc-warning); background:var(--mc-status-warning-bg); }
.developer-side { display:flex; flex-direction:column; gap:14px; } .developer-side>section { padding:16px; border:1px solid var(--mc-border); border-radius:var(--mc-radius-sm); background:var(--mc-bg-elevated); } .developer-side h3 { margin:5px 0 0; font-size:var(--mc-text-sm); } .capability-list { margin:13px 0 0; padding-left:17px; color:var(--mc-status-error-text); font-size:var(--mc-text-xs); line-height:1.6; } .capability-list li+li { margin-top:7px; }
.source-gate-card { min-height:120px; } .source-gate-head { display:flex; align-items:flex-start; justify-content:space-between; gap:10px; } .source-gate-state { flex:none; padding:3px 7px; border-radius:var(--mc-radius-xs); background:var(--mc-bg-muted); font-size:9px; font-weight:700; }
.source-scope { display:flex; align-items:center; gap:5px; margin:12px 0 8px; color:var(--mc-text-tertiary); font-size:9px; } .source-scope code { color:var(--mc-text-secondary); word-break:break-all; } .source-meta { display:flex; flex-wrap:wrap; gap:7px; font-size:9px; }
...
.acceptance-ladder li>span { display:grid; place-items:center; width:25px; height:20px; border-radius:4px; color:var(--mc-text-secondary); background:var(--mc-border-light); font:700 9px var(--mc-mono,monospace); }
.acceptance-ladder b,.acceptance-ladder small { display:block; } .acceptance-ladder b { font-size:9px; } .acceptance-ladder small { margin-top:3px; color:var(--mc-text-secondary); font-size:9px; line-height:1.45; }
.acceptance-ladder strong { font-size:8px; white-space:nowrap; } .next-source-action { margin:8px 0 0; padding:8px; border-radius:6px; color:var(--mc-text-secondary); background:var(--mc-status-info-bg); font-size:9px; line-height:1.5; } .next-source-action b { display:block; margin-bottom:2px; color:var(--mc-status-info-text); }
.signal-readiness-list { margin:12px 0; padding:0; list-style:none; } .signal-readiness-list li { display:grid; grid-template-columns:minmax(0,1fr) auto; gap:3px 8px; padding:7px 0; border-top:1px solid var(--mc-border-light); } .signal-readiness-list code { font-size:9px; } .signal-readiness-list span { font-size:9px; } .signal-readiness-list small { grid-column:1/-1; color:var(--mc-text-tertiary); font-size:9px; word-break:break-all; }
.source-blocker { margin:7px 0; padding:7px 8px; border-radius:var(--mc-radius-xs); color:var(--mc-status-error-text); background:var(--mc-status-error-bg); font-size:9px; line-height:1.5; } .gate-note { display:block; margin-top:9px; color:var(--mc-text-secondary); font-size:9px; line-height:1.55; }
.validation-result { margin:9px 0; padding:9px; border-radius:var(--mc-radius-sm); font-size:9px; } .validation-result.passed { color:var(--mc-status-success-text); background:var(--mc-status-success-bg); } .validation-result.blocked { color:var(--mc-status-warning-text); background:var(--mc-status-warning-bg); } .validation-result.pending { color:var(--mc-status-info-text); background:var(--mc-status-info-bg); } .validation-result b,.validation-result span,.validation-result small { display:block; } .validation-result span { margin-top:4px; } .validation-result small { margin-top:5px; color:var(--mc-text-secondary); line-height:1.45; }
.owner-acceptance-result code { font-size:9px; overflow-wrap:anywhere; } .owner-acceptance-result.passed { border-color:var(--mc-success); background:var(--mc-status-success-bg); } .owner-acceptance-result.blocked { border-color:var(--mc-warning); background:var(--mc-status-warning-bg); } .owner-acceptance-result.pending { border-color:var(--mc-border-light); background:var(--mc-status-info-bg); }
.spine-preview-result span { overflow-wrap:anywhere; line-height:1.45; }
.action-card { margin-top:12px; padding:12px; border:1px solid var(--mc-border); border-radius:var(--mc-radius-xs); } .action-card.write { border-color:var(--mc-danger-border); } .action-card>div { display:flex; justify-content:space-between; gap:8px; } .action-card code,.action-card>div span { color:var(--mc-text-secondary); font-size:9px; }
.action-card>b { display:block; margin-top:7px; font-size:var(--mc-text-xs); } .action-card>p { margin:4px 0 9px; color:var(--mc-text-secondary); font-size:10px; line-height:1.5; }
.active { color:var(--mc-primary)!important; } .success { color:var(--mc-success)!important; } .warning { color:var(--mc-warning)!important; }
.timeline-filter-bar { display:flex; gap:6px; margin:14px 0 14px; }
.timeline-filter-btn { padding:4px 12px; border:1px solid var(--mc-border); border-radius:var(--mc-radius-xs); background:var(--mc-bg-elevated); color:var(--mc-text-secondary); font-size:10px; cursor:pointer; transition:all .15s; }
.timeline-filter-btn:hover { border-color:var(--mc-primary); color:var(--mc-primary); }
.timeline-filter-btn.active { background:var(--mc-primary-bg, var(--mc-bg-muted)); border-color:var(--mc-primary); color:var(--mc-primary); font-weight:600; }
.timeline-phase-divider { display:flex; align-items:center; gap:12px; margin:20px 0 14px; color:var(--mc-text-tertiary); font-size:10px; font-weight:700; letter-spacing:.08em; text-transform:uppercase; }
...
.contrast-diff-title { font-size:var(--mc-text-sm, 12px); font-weight:600; }
.contrast-diff-status { font-size:10px; padding:2px 7px; border-radius:var(--mc-radius-xs); }
.contrast-diff-status.available { color:var(--mc-success); background:var(--mc-status-success-bg); }
.contrast-diff-container { background:var(--mc-bg-elevated); }
.contrast-diff-note { padding:8px 14px; font-size:10px; color:var(--mc-text-secondary); border-top:1px solid var(--mc-border); }
@media(max-width:1100px){.convergence-grid{grid-template-columns:1fr}}
</style>
